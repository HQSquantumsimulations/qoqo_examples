...
use roqoqo::measurements::{PauliZProductInput, PauliZProduct};
...
use ndarray::{array, Array, Array1, Array2, ArrayView1};
use nalgebra as na;
use na::ComplexField;
...
pub fn run_simple_vha() {
    ...
    fn create_even_hopping_circuit(thetasymb: CalculatorFloat, number_qubits: usize, hopping_parameter: f64) -> Circuit {
    ...
    // * number_qubits: number of qubits in the system.
    // * hopping_parameter: parameter 't' for the hopping term of the Hamiltonian.
    fn create_odd_hopping_circuit(thetasymb: CalculatorFloat, number_qubits: usize, hopping_parameter: f64) -> Circuit {
        let mut circuit = Circuit::new();
        ...
        circuit.add_operation(RotateZ::new(number_qubits - 1, (thetasymb * hopping_parameter).to_string().into()));
        circuit.add_operation(CNOT::new(0, number_qubits - 1));
        circuit.add_operation(Hadamard::new(number_qubits-1));
        circuit.add_operation(Hadamard::new(0));
    ...
    fn create_magnetic_field_circuit(thetasymb: CalculatorFloat, number_qubits: usize, magnetic_field: f64) -> Circuit {
        let mut circuit = Circuit::new();
    ...
    fn wrapper_function(theta: &[f64]) -> f64 {
        ...
        measurement_input.add_pauliz_product("ro_z".to_string(), vec![0]).unwrap();
        measurement_input.add_pauliz_product("ro_z".to_string(), vec![1]).unwrap();
        measurement_input.add_pauliz_product("ro_z".to_string(), vec![2]).unwrap();
        measurement_input.add_pauliz_product("ro_x".to_string(), vec![0,1]).unwrap();
        measurement_input.add_pauliz_product("ro_x".to_string(), vec![1,2]).unwrap();
        measurement_input.add_pauliz_product("ro_x".to_string(), vec![0,2]).unwrap();
